    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[calc(100vh-200px)]">
        <div class="space-y-4">
            <div class="flex flex-row gap-2 w-full">
                <div class="flex flex-col w-1/2 gap-3">
                    <div
                        class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 hover:shadow-md transition-shadow h-full">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-xs font-medium text-gray-600 dark:text-gray-400">Transaksi Hari Ini</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-white">
                                    {{ \App\Models\Transaction::whereDate('tanggal_transaksi', today())->count() }}</p>
                            </div>
                        </div>
                    </div>
                    <div
                ...
                <div class="flex w-full">
                    <div
                        class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 h-full overflow-hidden w-full">
                        <div
                            class="bg-green-50 dark:bg-green-900 px-6 py-4 border-b border-green-100 dark:border-green-800">
                                    ...
                                    @foreach ($topItems as $index => $item)
                                        @if ($item->barang)
                                            <div class="flex flex-row p-3 bg-green-50 dark:bg-green-800 rounded-lg">
                                                <div
                                                    class="w-6 h-6 bg-green-100 dark:bg-green-700 rounded-full flex items-center justify-center mr-3 flex-shrink-0">
                                                    <span
                                                        class="font-medium text-green-600 dark:text-green-200">{{ $index + 1 }}</span>
                                                </div>
                                                <div class="flex-1 min-w-0">
                                                    <p class="font-medium text-gray-800 dark:text-white text-sm truncate">
                                                        {{ $item->barang->nama_barang }}</p>
                                                    <p class="text-xs text-gray-600 dark:text-gray-400">{{ $item->total }} terjual
                                                    </p>
                                                </div>
                                            </div>
                                        @endif
            ...
            <div class="lg:col-span-2">
                <div
                    class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 h-full overflow-hidden">
                    ...
                    <div class="p-6 overflow-y-auto" style="height: calc(100% - 80px);">
                        ...
                        @if($stokMenipis->count() > 0)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach ($stokMenipis as $product)
                                    <div
                                        class="p-4 bg-red-50 dark:bg-red-800 rounded-lg border border-red-100 dark:border-red-700">
                                        <div class="flex items-center justify-between">
                                            <div class="flex-1">
                                                <p class="font-medium text-gray-900 dark:text-white text-sm">
                                                    {{ $product->nama_barang }}</p>
                                                <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">Kode Barang:
                                                    {{ $product->id ?? 'N/A' }}</p>
                                            </div>
                                            <span
                                                class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 dark:bg-red-700 text-red-800 dark:text-red-200 ml-2">
                                                {{ $product->stok }}
                                            </span>
                                        </div>
                                    </div>
                        ...
                        @else
                            <div class="text-center py-16">
                                <svg class="w-16 h-16 mx-auto text-green-400 mb-4" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <p class="text-gray-500 dark:text-gray-400 font-medium">Semua stok aman!</p>
                                <p class="text-sm text-gray-400 dark:text-gray-500">Tidak ada barang dengan stok kurang dari
                                    10</p>
                            </div>
                        @endif
                    </div>
        ...
        <div class="space-y-6">
            <div class="flex items-center gap-3">
                <input disabled wire:model="prompt" type="text" placeholder="Pertanyaan ke Gemini"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500" />

                <x-filament::button wire:click="generateFromGemini"
                    class="px-4 py-2 font-semibold transition duration-150 ease-in-out rounded-md">
                    Tanya Gemini
                </x-filament::button>
            </div>
            @if ($jawaban)
                <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    ...
                    <div class="text-gray-700 dark:text-gray-200 whitespace-pre-line leading-relaxed">
                        {{ $jawaban }}
                    </div>
                </div>
            @endif
